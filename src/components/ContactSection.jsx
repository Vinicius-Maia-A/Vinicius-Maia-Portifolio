...
export default function ContactSection() {
  const methods = [
    { icon: "📧", label: "E-MAIL", val: DATA.contact.email, href: `mailto:${DATA.contact.email}` },
    { icon: "💼", label: "LINKEDIN", val: DATA.contact.linkedin, href: `https://${DATA.contact.linkedin}` },
    { icon: "🐙", label: "GITHUB", val: DATA.contact.github, href: `https://${DATA.contact.github}` },
    { icon: "💬", label: "WHATSAPP", val: DATA.contact.whatsapp, href: DATA.contact.whatsappLink },
  ];
  return (
    <div id="s-contact">
      <TermWindow title="bash — ~/contact" promptCmd="open" promptArg="--channel" promptFlag="all">
        <div className="output-block">
          <div className="sec-label">// CANAIS DE CONTATO</div>
          <div className="contact-grid">
            {methods.map((m) => (
              <div key={m.label} className="contact-method" onClick={() => window.open(m.href, "_blank")}>
                <div className="contact-icon">{m.icon}</div>
